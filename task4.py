...
class Lecturer(Mentor):
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, Student) or isinstance(other, Reviewer):
            print('Это не лектор')
        else:
            return self.average_grade < other.average_grade
...
def average_grade_student(list_student, course):
   result = sum([sum(student.grades[course]) for student in list_student]) / sum([len(student.grades[course]) for student in list_student])
   print(result)

def average_grade_lecturer(list_lecturer, course):
   result = sum([sum(lecturer.grades[course]) for lecturer in list_lecturer]) / sum([len(lecturer.grades[course]) for lecturer in list_lecturer])
   print(result)
...
print()
average_grade_lecturer([lecturer_1, student_2], 'Python')